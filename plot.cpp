...
//    setAxisScaleEngine(QwtPlot::yLeft, scaleEngineLeft);
    ...
    s_curve->setStyle(QwtPlotCurve::Lines);
    s_curve->setPen(QPen(Qt::red, 3.0));
...
void Plot::updateCurve()
{
    ...
    if(totalPoints > d_paintedPoints)
    {
        ...
        d_directPainter->drawSeries(s_curve, d_paintedPoints-1, totalPoints-1);
...
void Plot::appendPoint(QPointF point1, QPointF point2)
{
    CurveData *data1 = static_cast<CurveData *>(d_curve->data());
    CurveData *data2 = static_cast<CurveData *>(s_curve->data());
    data1->append(point1);
    data2->append(point2);
    int totalPoints = data1->size();
...
{
    if( event->timerId() == d_timerId && flagUpd)
    {
